...
resource "null_resource" "ai_jar_coaching_lambda_source_hash" {
  triggers = {
    source_code_hash = md5(join("", [
      for f in fileset("${path.module}/../lambda/ai_jar_coaching", "**/*.py") :
      filemd5("${path.module}/../lambda/ai_jar_coaching/${f}")
    ]))
  }
}
...
resource "aws_lambda_function" "ai_jar_coaching" {
  ...
  source_code_hash = null_resource.ai_jar_coaching_lambda_source_hash.triggers.source_code_hash
